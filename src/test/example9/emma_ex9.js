...
// Widgets and the like
  ...
  Widget.prototype.addListener = function (listener) {
    this.listeners.push(listener)
  }

  Widget.prototype.removeListener = function (listener) {
    this.listeners = _.without(this.listeners, listener);
  }

  Widget.prototype.setMode = function (mode) {
    if (mode === Widget.VIEW_MODE) {
      this.mode = Widget.VIEW_MODE;
    } else if (mode === Widget.EDIT_MODE) {
      this.mode = Widget.EDIT_MODE;
    }
  ...
  var _Table = function (adapterFactory, container, input, template) {
    ...
    Table.prototype.renderColumn = function (column, adapter) {
      if (column instanceof CustomColumn) {
        return column.render();
      } else {
        if (this.mode === Widget.EDIT_MODE) {
          return adapter.getPropertyById(column.key).getCellEditor().render().html();
        } else {
          return adapter.getPropertyById(column.key).getValue();
        }
      }
    }
    ...
    Table.prototype.render = function (input) {
      ...
      input.getContents().forEach(function (object) {
        ...
        input.getColumns().forEach(function (column) {
          rowValues.push(self.renderColumn(column, adapter));
        });
  ...
  var CustomColumn = function (key, displayName, generator) {
    this.key = key;
